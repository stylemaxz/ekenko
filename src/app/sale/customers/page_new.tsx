    ...
    <div className="pb-24 pt-6 px-4">
      ...
      <div className="space-y-4">
          {filteredCompanies.map(company => (
              <div key={company.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
          ...
          {filteredCompanies.length === 0 && (
              <div className="text-center py-12 text-slate-400">
                  ...
                  <p>{language === 'th' ? 'ไม่พบลูกค้า' : 'No customers found'}</p>
              </div>
          )}
      </div>

      {/* Add Customer Modal - keeping existing implementation */}
      {/* ... (previous add modal code) ... */}
